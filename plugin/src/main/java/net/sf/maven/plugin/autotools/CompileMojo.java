...
public final class CompileMojo
   ...
   private void autogen()
      ...
      if(!autogenScript.exists()) {
         throw new MojoExecutionException("autogen script does not exist: " + autogenScript.getAbsolutePath());
      }
      if (!autogenScript.canExecute()) {
         autogenScript.setExecutable(true);
      }
      String[] shellCommand = {
            "sh", "-c", autogenScript.getAbsolutePath()
      };
      if (verbose && getLog().isInfoEnabled()) {
